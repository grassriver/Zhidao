...
from Ratios import *
from VaR import *
from weekly_monthly_returns import *
import matplotlib.pyplot as plt
...
class Portfolio(object):
    ...
    def weekly_returns(self):
        ...
        return weekly_return(price)
        
    def monthly_returns(self):
        port = pd.DataFrame(self.port_daily_balance())
        price = pd.merge(self.stock_price(),port, left_index=True, right_index=True)
        return monthly_return(price)
    ...
    def performance_matrix(self, annualization=252):
        ...
        matrix = pd.DataFrame({'Beta': ret_price.apply(get_beta, market=ret_index),
                                   ...
                                   'Market Correlation': ret_price.apply(get_correlation, market=ret_index),
                                   ...
                                   'Historical VaR': ret_price.apply(hist_var),
                                   'Delta-Normal VaR': ret_price.apply(delta_normal_var),
                                   'EWMA VaR': ret_price.apply(ewma_var),
                                   'Conditional VaR': ret_price.apply(conditional_var),
